...
const UpdateTask = ({title, setTitle,description, setDescription, priority, setPriority,complete, setComplete, idTask}) => {
    ...
    const updateTask = async (event) => {
        ...
        axios.put(`http://localhost:3200/task/updateTask/${idTask}`, list)
            .then((res) => {
                Swal.fire({
                    icon: 'success',
    ...
    return (
        <Grid container component="main" sx={{ height: '100vh' }}>
            <Grid item xs={12} sm={8} ml={40} md={6} sx={{ boxShadow: "none" }}>
                <Box component="form" noValidate onSubmit={updateTask}
                    sx={{
                        mb: 5,
                        mt: 11,
                        mx: 4,
